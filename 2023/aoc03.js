...
class Field {
    constructor(x, y) {
        this.x = x
        this.y = y
        this.value = data[y].charAt(x)
        this.id = getId(x, y)
        this.neighbours = []
    }

    getNeighbours() {
        const startX = Math.max(0, this.x - 1)
        const startY = Math.max(0, this.y - 1)
        const endX = Math.min(maxX, this.x + 1)
        const endY = Math.min(maxY, this.y + 1)
        ...
        for (let i = startX; i <= endX; i++) {
            for (let j = startY; j <= endY; j++) {
                ...
                this.neighbours.push(fieldsMap[id])
            }
        }
    }
}
...
class Word {
    constructor(field) {
        ...
        this.processedIds = []
        this.unprocessedFields = [field]
        this.processFields()
        this.getValue()
    }

    processFields() {
        while (this.unprocessedFields.length > 0) {
            const field = this.unprocessedFields.shift()

            if (this.processedIds.includes(field.id)) continue

            this.processedIds.push(field.id)

            if (field.value === '.') {
                continue
            } else if (/[0-9]/.test(field.value)) {
                this.numberFields.push(field)
                this.unprocessedFields.push(...field.neighbours)
            } else {
                console.assert(this.symbolField === null)
                this.symbolField = field
            }
        }
    }

    getValue() {
        this.numberFields.sort((a, b) => a.id.localeCompare(b.id))
        const stringValue = this.numberFields.reduce((numeric, field) => `${numeric}${field.value}`, '')
        this.value = Number (stringValue)
    }
}

const numberFields = Object.values(fieldsMap).filter(field => /[0-9]/.test(field.value))
const processedIds = []
const words = []

numberFields.forEach(field => {
    if (processedIds.includes(field.id)) return
    const word = new Word(field)
    words.push(word)
    processedIds.push(...word.processedIds)
})

const sum = words.filter(word => word.symbolField).reduce((sum, word) => sum + word.value, 0)

console.log(sum)

const gears = Object.values(fieldsMap).filter(field => field.value === '*')

let sum2 = 0
gears.forEach(gear => {
    const foundWords = []
    words.forEach(word => {
        if (word.symbolField?.id === gear.id) {
            foundWords.push(word)
        }
    })

    if (foundWords.length === 2) {
        const ratio = foundWords[0].value * foundWords[1].value
        sum2 += ratio
    }
})
