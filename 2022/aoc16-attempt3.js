...
class Solver {
    ...
    getNewSolvers() {
        ...
        Object.keys(this.currentValve.distanceToValve)
            .filter((valveName) => !this.hasOpened(valveName) && !valveNamesToIgnore.includes(valveName))
            .forEach((valveName) => {
...
// part 1
const part1Solver = getBestSolverForRemainingTime(29)
console.log(part1Solver.score)

while (solvers.length) solvers.shift()
while (visitedStates.length) visitedStates.shift()
...
valveNamesToIgnore.push(...bestSolver.openedValves)

const otherSolver = getBestSolverForRemainingTime(25)

console.log(bestSolver.score + otherSolver.score)
