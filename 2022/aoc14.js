...
class Sand extends Obstruction {
    fall() {
        let isFalling = true
        const obstructions = [...stones, ...sand]

        while (isFalling) {
            const sourceObstruction = obstructions.find(obstruction => obstruction.x === 500 && obstruction.y === 0)
            if (sourceObstruction) return false

            const below = obstructions.find(obstruction => obstruction.x === this.x && obstruction.y === this.y + 1)
            const belowLeft = obstructions.find(obstruction => obstruction.x === this.x - 1 && obstruction.y === this.y + 1)
            const belowRight = obstructions.find(obstruction => obstruction.x === this.x + 1 && obstruction.y === this.y + 1)
            if (!below) {
                this.y++
            } else if (!belowLeft) {
                this.y++
            ...
            if (this.y > maxY) return hasFloor
        }

    }
}

input.forEach(stonePath => {
    const [first, ...rest] = stonePath

    let x = first[0]
    let y = first[1]
    stones.push(new Obstruction(x, y))

    rest.forEach(([cornerX, cornerY]) => {
        if (cornerX === x && cornerY > y) {
            for (let i = y+1; i <= cornerY; i++) {
                stones.push(new Obstruction(x, i))
            }
        } else if (cornerX === x && cornerY < y) {
            for (let i = y-1; i >= cornerY; i--) {
                stones.push(new Obstruction(x, i))
            }
        } else if (cornerX > x && cornerY === y) {
            for (let i = x+1; i <= cornerX; i++) {
                stones.push(new Obstruction(i, y))
            }
        } else if (cornerX < x && cornerY === y) {
            for (let i = x-1; i >= cornerX; i--) {
                stones.push(new Obstruction(i, y))
...
let done = false
while (!done) {
    const sandUnit = new Sand(500, 0)
    let cameToRest = sandUnit.fall()
    if (cameToRest) {
        sand.push(sandUnit)
    } else {
        done = true
