...
let heigth = floor.length
let width = floor[0].length
...
class Node {
    ...
    evaluateRoute(distance) {
        if (this.distance > this.score + distance) {
            this.distance = this.score + distance
        }
    }

    evaluateNeighbours() {
        this.neighbours.forEach((node) => !node.done && node.evaluateRoute(this.distance))
        this.done = true
        if (this.isEnd) console.log(this.distance)
    }
}

let grid = new Array(heigth * 5)
for (let i=0; i<grid.length; i++) {
    grid[i] = new Array(width * 5)
    for (let j = 0; j < grid[i].length; j++) {
        grid[i][j] = new Node(((floor[i%heigth][j%width] + Math.floor(i / heigth) + Math.floor(j / width) - 1) % 9) + 1)
    }
}
...
for (let i=0; i<grid.length; i++) {
    for (let j = 0; j < grid[i].length; j++) {
        ...
        if (i < grid.length - 1) grid[i][j].addNeighbour(grid[i + 1][j])
        if (j > 0) grid[i][j].addNeighbour(grid[i][j - 1])
        if (j < grid[i].length - 1) grid[i][j].addNeighbour(grid[i][j + 1])
    }
}

grid[0][0].distance = 0
grid[width * 5 - 1][heigth * 5 - 1].isEnd = true

let nodes = grid.reduce((p, c) => [...p, ...c], []).sort((a, b) => a.distance - b.distance)

while (nodes.length) {
    let [first, ...rest] = nodes
    first.evaluateNeighbours()
    nodes = rest.sort((a, b) => a.distance - b.distance)
}
