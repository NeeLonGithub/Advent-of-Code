...
function isOne(digit) {
    return digit.length === 2
}

function isSeven(digit) {
    return digit.length === 3
}

function isFour(digit) {
    return digit.length === 4
}

function isEight(digit) {
    return digit.length === 7
}

// 6 segments
function isSixByOne(digit, one) {
    if (digit.length !== 6) return false;
    return digit.split('').filter((segment) => one.includes(segment)).length === 1;
}

function isSixBySeven(digit, seven) {
    if (digit.length !== 6) return false;
    return digit.split('').filter((segment) => seven.includes(segment)).length === 2;
}

function isNineByFour(digit, four) {
    if (digit.length !== 6) return false;
    return digit.split('').filter((segment) => four.includes(segment)).length === 4;
}

function isZeroByOneAndFour(digit, one, four) {
    if (digit.length !== 6) return false;
    return !isSixByOne(digit, one) && !isNineByFour(digit, four);
}

// 5 segments
function isTwoByFour(digit, four) {
    if (digit.length !== 5) return false;
    return digit.split('').filter((segment) => four.includes(segment)).length === 2;
}

function isThreeByOne(digit, one) {
    if (digit.length !== 5) return false;
    return digit.split('').filter((segment) => one.includes(segment)).length === 2;
}

function isThreeBySeven(digit, seven) {
    if (digit.length !== 5) return false;
    return digit.split('').filter((segment) => seven.includes(segment)).length === 3;
}

function isFiveByOneAndFour(digit, one, four) {
    if (digit.length !== 5) return false;
    return !isThreeByOne(digit, one) && !isTwoByFour(digit, four);
}

function isFiveByFourandSeven(digit, four, seven) {
    if (digit.length !== 5) return false;
    return !isThreeBySeven(digit, seven) && !isTwoByFour(digit, four);
}

function getNumbers(digits) {
    let numbers = new Map();
    digits.forEach((digit) => {
       if (isOne(digit)) numbers.set(1, digit.split('').sort().join(''));
       if (isFour(digit)) numbers.set(4, digit.split('').sort().join(''));
       if (isSeven(digit)) numbers.set(7, digit.split('').sort().join(''));
       if (isEight(digit)) numbers.set(8, digit.split('').sort().join(''));
    });

    digits.forEach((digit) => {
       if (numbers.has(1) && !numbers.has(6) && isSixByOne(digit, numbers.get(1))) numbers.set(6, digit.split('').sort().join(''));
       if (numbers.has(1) && !numbers.has(3) &&  isThreeByOne(digit, numbers.get(1))) numbers.set(3, digit.split('').sort().join(''));
       if (numbers.has(4) && !numbers.has(2) &&  isTwoByFour(digit, numbers.get(4))) numbers.set(2, digit.split('').sort().join(''));
       if (numbers.has(4) && !numbers.has(9) &&  isNineByFour(digit, numbers.get(4))) numbers.set(9, digit.split('').sort().join(''));
       if (numbers.has(7) && !numbers.has(3) &&  isThreeBySeven(digit, numbers.get(7))) numbers.set(3, digit.split('').sort().join(''));
       if (numbers.has(7) && !numbers.has(6) &&  isSixBySeven(digit, numbers.get(7))) numbers.set(6, digit.split('').sort().join(''));
    });

    digits.forEach((digit) => {
        let sortedDigit = digit.split('').sort().join('');
        if (sortedDigit.length === 5) {
            if (numbers.has(2) && numbers.has(3) && numbers.get(2) !== sortedDigit && numbers.get(3) !== sortedDigit) numbers.set(5, sortedDigit);
            if (numbers.has(2) && numbers.has(5) && numbers.get(2) !== sortedDigit && numbers.get(5) !== sortedDigit) numbers.set(3, sortedDigit);
            if (numbers.has(3) && numbers.has(5) && numbers.get(3) !== sortedDigit && numbers.get(5) !== sortedDigit) numbers.set(2, sortedDigit);
        }
        if (sortedDigit.length === 6) {
            if (numbers.has(0) && numbers.has(6) && numbers.get(0) !== sortedDigit && numbers.get(6) !== sortedDigit) numbers.set(9, sortedDigit);
            if (numbers.has(0) && numbers.has(9) && numbers.get(0) !== sortedDigit && numbers.get(9) !== sortedDigit) numbers.set(6, sortedDigit);
            if (numbers.has(6) && numbers.has(9) && numbers.get(6) !== sortedDigit && numbers.get(9) !== sortedDigit) numbers.set(0, sortedDigit);
        }
    })
...
let result = input.reduce((count, [data, result]) => {
    let numbers = getNumbers([...data, ...result]);
    let subCount = 0;
    numbers.forEach((digit, number) => {
        if (result[0].split('').sort().join('') === digit) subCount += 1000 * number;
        if (result[1].split('').sort().join('') === digit) subCount += 100 * number;
        if (result[2].split('').sort().join('') === digit) subCount += 10 * number;
        if (result[3].split('').sort().join('') === digit) subCount += number;
    });
